...
public class Customer {
    ...
    public Transaction withdraw(double amount) {
        ...
        if(amount>=50000){
            throw new IllegalArgumentException("Withdrawal denied : can not withdraw more than 50000");
        }
        if (amount+500 > balance) {
            throw new IllegalStateException(
                    String.format("Withdrawal denied: amount (%.2f) exceeds balance (%.2f).", amount, balance)
            );
        }
        balance -= amount;
    ...
    public void transfer(double amount, Customer recipient) {
        ...
        if (amount+500 > balance) {
            throw new IllegalStateException(
                    String.format("Transfer denied: amount (%.2f) exceeds balance (%.2f).", amount, balance)
            );
        }
        if(amount>50000){
                throw new IllegalArgumentException("Transfer denied : cannot transfer more than 50000");
        }

        this.balance -= amount;
        Transaction outTxn = new Transaction("transfer_out", amount, this.balance);
        this.transactionHistory.add(outTxn);

        recipient.balance += amount;
        Transaction inTxn = new Transaction("transfer_in", amount, recipient.balance);
